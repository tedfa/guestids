# Still need to add error checking to IDs entered section. What if I enter a number out of range
# or if someone else resets a password before I can get to it and my dictionary is then wrong
# Also need error checking for
import pickle
import webscripts as web
...
def load_dict_data(array, newpass):
    ...
    for elems in array:
        ...
        web.selenium_script(oldpass,newpass, elems)
    for elms in array:
        netids[elms] = newpass
...
# to the 'netids' dictionary. That will then be saved to a file via pickle.
